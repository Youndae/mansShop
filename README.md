...
> 초기 버전인 JSP 버전으로 프로젝트 마무리 이후 상태 유지 중입니다.   
> 마무리 이후 진행한 Spring Boot 버전의 경우 지속적인 리팩토링을 진행하고 있습니다.   
> 쇼핑몰에서 볼 수 있는 기본적인 상품 정보, 장바구니, 결제, 마이페이지, 로그인 등의 기능들을 구현했으며, 관리자의 기능들은 예상되는 기능들을 설계해 구현했습니다.
...
<strong>4. [페이지별 기능 상세](#페이지별-기능-상세)</strong>   
<strong>5. [기능 및 개선 내역](#기능-및-개선-내역)</strong>

<br/>

# 프로젝트 구조
<img src="./src/main/webapp/resources/README_image/project_structure.jpg"/>
...
|Category|Tech Stack|
|---|---|
|Backend| - Spring 5.0.7 <br/> - JDK 8 <br/> - Maven <br/> - MyBatis <br/> - SpringSecurity <br/> - Java Mail|
|Frontend| - JSP <br/> - JQuery <br/> - Ajax <br/> - I'mport API <br/> - Kakao 우편번호 서비스 API|
|Database| - Oradle 18c <br/> - Redis|
|Environment| - IntelliJ <br/> - GitHub <br/> - Docker( Redis )|
...
- 장바구니
  ...
  - 장바구니 상품 선택 또는 전체 구매
  - 장바구니 상품 선택 또는 전체 삭제
- 주문 조회(비로그인시에만 출력. 로그인 시 마이페이지에서 조회 가능)
- 로그인
  - 회원가입
  - 로그인
  - 아이디 및 비밀번호 찾기
...
- 매출 관리
  - 기간별 매출
    - 선택연도 월별 매출 목록 및 연매출, 판매량, 주문량
    - 월 매출 상세 정보
      - 베스트5 상품, 분류별 매출, 일별 매출
      - 분류별 상세 매출
      - 선택 날짜의 전체 주문 내역 조회
  - 상품별 매출
    - 상품별 매출 목록
...
* <strong>백엔드</strong>
  1. [인증 인가 처리](#인증-인가-처리)
  2. [Oracle MyBatis 환경에서 INSERT ALL 문제 발생 및 해결](#Oracle-MyBatis-환경에서-INSERT-ALL-문제-발생-및-해결)
  3. [Multipart formData 요청에서의 문제](#Multipart-formData-요청에서의-문제)
  4. [주문 페이지 접근 시 데이터 처리](#주문-페이지-접근-시-데이터-처리)
...
이 문제 해결 방법에 대해서는 블로그에 좀 더 상세하게 기록해두었으며, 아래 링크를 통해 확인하실 수 있습니다.

<a href="https://myyoun.tistory.com/240">Oracle 다중 insert 처리 문제 해결</a>
...
데이터 양이 너무 많다면 UX 개선을 위해 getJSON으로 나눠서 요청하는게 효과가 좋을 수 있겠으나, 그렇지 않은 페이지라고 생각했고, 단지 데이터 파싱을 처리하기 위해 나눠서 요청하는 것은 비효율적이라고 생각했습니다.
기존에 JQuery를 통해 데이터를 파싱하던 작업을 JSTL로 처리하는 방향으로 수정했으며, 주로 c, fmt 태그를 가장 많이 사용했고, sec 태그는 간간히 추가해서 사용했습니다.
...
<a href="https://github.com/Youndae/mansShop/blob/master/src/main/webapp/WEB-INF/views/product/productDetail.jsp">productDetail.jsp 전체 코드</a>
